        ...
        <span :class="props.questions[currentIndex]?.isCorrect ? 'text-success' : 'text-danger'">
          {{
            props.questions[currentIndex]?.isCorrect
              ? '答對了！'
              : '答錯了！正確答案是：' + props.questions[currentIndex]?.Answer
          }}
        </span>
        <!-- maybe 詳解？-->
...
const currentImage = computed(() => props.questions[currentIndex.value]?.Content_pic || '')

//const currentImage = computed(() => props.questions[currentIndex.value]?.Contentpic || '')
...
onMounted(() => {
  if (props.questions && props.questions.length > 0) {
    console.log('❤️ 題目載入成功:', props.questions)
    //currentLength.value = props.questions.length
    console.log('📦 問題數量:', currentLength.value)

    console.log('🚀 currentType', currentType)
    console.log('👀 顯示條件：', props.questions, currentLength.value)

    console.log('🧠 currentQuestion:', currentQuestion.value)
    console.log('🖼 currentImage:', currentImage.value)
    console.log('📏 currentLength:', currentLength.value)
...
onMounted(() => {
  ...
  console.log('📦 問題數量currentLength.value:', currentLength.value)
